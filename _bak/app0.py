...
def build_summary(results: list[dict]) -> pd.DataFrame:
    ...
    for r in results:
        ...
        # ✅ If no allocation (e.g., Task 4/5/6 descriptive), leave coverage blank
        if allocation is None:
            coverage_cell = ""
        else:
...
if run_all:
    # ---- Task 1 ----
    t1 = ensure_task_row(solve_task1(), "Task 1", "Done")
    ...
    # ---- Task 4 ----
    t4 = ensure_task_row(solve_task4(), "Task 4", "Done")
    t4["case"] = "Random sample (n=400)"
    t4["download_df"] = t4.get("download_df", pd.DataFrame())

    # ---- Task 5 ----
    t5 = ensure_task_row(solve_task5(), "Task 5", "Done")
...
st.subheader("Summary (All Tasks 1–6)")
summary_df = build_summary(results)
st.dataframe(summary_df, use_container_width=True)

st.caption(
    "Note: Tasks 4–6 are descriptive/analysis tasks, so optimisation KPIs (e.g., cost/coverage) may be blank."
)
...
with tab6:
    ...
    st.dataframe(t5["tables"]["table_5_1"], use_container_width=True)

    st.subheader("Table 5.2 – Numeric variables vs Breach")
    st.dataframe(t5["tables"]["table_5_2"], use_container_width=True)

    st.subheader("Table 5.3 – Categorical variables vs Breach")
    st.dataframe(t5["tables"]["table_5_3"], use_container_width=True)

    st.divider()

    # --- Figures
    st.subheader("Figure 5.0 – Overview of breach context")
    st.pyplot(t5["figures"]["fig0"], clear_figure=False)

    st.subheader("Figure 5.1 – Key factors associated with breaches")
    st.pyplot(t5["figures"]["fig1"], clear_figure=False)
